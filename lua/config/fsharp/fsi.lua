-- lua/config/fsharp/fsi.lua
-- F# Interactive terminal integration (Alt-Enter to send, Alt-@ to
-- toggle). setup() is once-guarded and registers the :Fsi* commands;
-- toggle() is exposed for the ftplugin keymap.
...
-- Wait until FSI prompt/banner appears before first send to avoid
-- pre-banner echo.
...
--- Toggle the FSI window/job (mapped to <M-@> in F# buffers).
function M.toggle()
...
function M.setup()
  ...
  -- Backwards compatibility: older sessions/mappings referenced the
  -- global. Prefer require("config.fsharp.fsi").toggle().
  _G._FSharpToggleFsi = M.toggle
end
