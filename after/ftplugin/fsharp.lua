...
vim.api.nvim_set_hl(0, "@module.builtin.fsharp",      { fg = "#f9e2af", italic = true })
vim.api.nvim_set_hl(0, "@lsp.type.module.fsharp",     { fg = "#f9e2af", italic = true })
vim.api.nvim_set_hl(0, "@lsp.type.namespace.fsharp",  { fg = "#f9e2af", italic = true })
...
if not vim.g._fsharp_fsi_loaded then
  ...
  local function open_fsi(keep_focus_in_repl)
    ...
    if not running() or not vim.api.nvim_win_is_valid(fsi.win or -1) then
      ...
      if type(jid) ~= "number" or jid <= 0 then
        vim.api.nvim_echo({ { "Failed to start FSI.", "ErrorMsg" } }, false, { err = true })
        return false
      end
      ...
      just_started = true
      log("open_fsi: win=%s buf=%s job=%s", tostring(fsi.win), tostring(fsi.buf), tostring(fsi.job))

      if not keep_focus_in_repl and vim.api.nvim_win_is_valid(original_win) then
      ...
      vim.api.nvim_create_autocmd({ "TermClose", "BufWipeout" }, {
        buffer = fsi.buf, once = true,
        callback = function()
          log("cleanup: terminal closed/wiped")
          if running() then pcall(vim.fn.jobstop, fsi.job) end
          fsi = { buf = nil, win = nil, job = nil, sent_cd = false }
        end,
      })
      vim.api.nvim_create_autocmd("WinClosed", {
        pattern = tostring(fsi.win), once = true,
        callback = function()
      ...
      if vim.api.nvim_buf_is_valid(fsi.buf) then
        vim.api.nvim_buf_call(fsi.buf, function() vim.cmd("normal! G") end)
      end
    end
  ...
  local function wait_until_ready_then(fn)
    ...
    local function ready()
      if not (fsi.buf and vim.api.nvim_buf_is_valid(fsi.buf)) then return false end
      ...
      for _, L in ipairs(lines) do
        if L:match("^%s*>%s*$") or L:find("For help type #help;;", 1, true) then
          return true
        end
      end
      return false
  ...
  local function send_lines(lines, opts)
    ...
    local function do_send()
      ...
      if opts.move_down then
        vim.schedule(function() pcall(vim.cmd, "normal! j") end)
      end
    end
  ...
  vim.api.nvim_create_user_command("FsiSend", function(args)
    ...
    if args.range and args.range > 0 then
      vim.schedule(function() pcall(vim.cmd, "normal! gv") end)
    end
  end, { range = true })

  -- Optional helpers
  vim.api.nvim_create_user_command("FsiShow", function() open_fsi(false) end, {})
  vim.api.nvim_create_user_command("FsiStatus", function()
    local msg = string.format(
      "STATUS: win=%s buf=%s job=%s running=%s sent_cd=%s log=%s",
      tostring(fsi.win), tostring(fsi.buf), tostring(fsi.job),
      tostring(running()), tostring(fsi.sent_cd), LOG_PATH
    )
    ...
    vim.defer_fn(function()
      if fsi.buf and vim.api.nvim_buf_is_valid(fsi.buf) then
        ...
        for _, L in ipairs(vim.api.nvim_buf_get_lines(fsi.buf, 0, -1, false)) do
          if L:find(tag, 1, true) then ok = true; break end
        end
        log("SelfTest: %s", ok and "PASS (tag found)" or "FAIL (tag not found)")
        vim.api.nvim_echo({ { ok and "SelfTest: PASS" or "SelfTest: FAIL", ok and "MoreMsg" or "ErrorMsg" } }, false, {})
      end
    end, 250)
  end, {})
  ...
  _G._FSharpToggleFsi = function()
    if running() and vim.api.nvim_win_is_valid(fsi.win or -1) then
      if fsi.job and running() then pcall(vim.fn.jobstop, fsi.job) end
      if vim.api.nvim_win_is_valid(fsi.win) then pcall(vim.api.nvim_win_close, fsi.win, true) end
      fsi = { buf = nil, win = nil, job = nil, sent_cd = false }
...
vim.api.nvim_create_autocmd("FileType", {
  ...
  callback = function(args)
    ...
    for _, client in pairs(vim.lsp.get_clients({ bufnr = args.buf })) do
      if client.name == "fsautocomplete" then return end
    end

    local fname = vim.api.nvim_buf_get_name(args.buf)
    local util  = require("lspconfig.util")
    local root  = util.root_pattern("*.sln", "*.fsproj", ".git")(fname) or vim.fs.dirname(fname)
